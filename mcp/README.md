...
把 [`hustpass`](../hustpass) 的**聚合层**（`client.aggregate`）包成一个 MCP 服务端，
...
## 设计取舍：为什么没有「校园网认证」MCP

本服务只聚合校园业务数据，没有做校园网认证（net mcp），原因有二：

1. **先有鸡还是先有蛋的逻辑悖论**：绝大多数用户使用云端大模型（Claude、ChatGPT 等）。当校园网断开、处于离线状态时，外部网络请求根本发不出去，远端模型压根无法接收指令，更不可能替你调用 MCP 工具去登录网络；即便用本地大模型，直接跑命令也远比过一遍大模型工具调用更直接可靠。
2. **轻量与必要性**：校园网认证本身只需在终端执行一行 `hustnet login`（或挂一个后台 `keepalive` 守护）。
