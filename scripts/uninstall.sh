...
if [ -f "${TARGET_BIN}" ]; then
  # 只删除本工具生成的 wrapper（含 hustnet.ts 标记），避免误删同名无关文件
  if grep -q "hustnet/bin/hustnet.ts" "${TARGET_BIN}" 2>/dev/null; then
    rm -f -- "${TARGET_BIN}"
    echo "==> 已成功移除: ${TARGET_BIN}"
  else
    echo "拒绝删除: ${TARGET_BIN} 不是本工具生成的 wrapper（未找到 hustnet.ts 标记）。" >&2
