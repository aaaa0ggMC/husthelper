...
export interface RequestOptions extends AxiosRequestConfig {
  omitCookies?: boolean;
  /**
   * 绑定的本机源地址（多网卡场景选哪张网卡）；未显式给 agent 时，
   * 会用它在内部构造 http/https Agent。axios 本身不认这个字段。
   */
  localAddress?: string;
}
...
export class Session {
  ...
  async request<T = any>(config: RequestOptions): Promise<AxiosResponse<T>> {
    const { omitCookies, localAddress, ...rest } = config;
    const headers = { ...(rest.headers ?? {}) };
    if (!omitCookies) headers.Cookie = this.cookieHeader(rest.url ?? "");
    if (localAddress && !rest.httpAgent && !rest.httpsAgent) {
      rest.httpAgent = new http.Agent({ localAddress, keepAlive: true });
      rest.httpsAgent = new https.Agent({ localAddress, keepAlive: true });
    }
